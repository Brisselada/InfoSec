...
def F_round(a, b, c, Wi, m):
    (a,b,c) = inner_round(a,b,c, Wi[0], m)
    (b,c,a) = inner_round(b,c,a, Wi[1], m)
    (c,a,b) = inner_round(c,a,b, Wi[2], m)
    (a,b,c) = inner_round(a,b,c, Wi[3], m)
    (b,c,a) = inner_round(b,c,a, Wi[4], m)
    (c,a,b) = inner_round(c,a,b, Wi[5], m)
    (a,b,c) = inner_round(a,b,c, Wi[6], m)
    (b,c,a) = inner_round(b,c,a, Wi[7], m)
    return (a, b, c)
...
def outer_round(message, a, b, c):
	x = []

	for j in range(0, 8):
		if isinstance(message, array.array):
			message = message.tobytes()
		elif isinstance(message, str):
			message = message.encode()
		tmp = int.from_bytes(message[j*8:(j+1)*8], byteorder='little')
		x.append(tmp)
	...
	for i in range(0, 3):
		if i != 0:
			x = keySchedule(x)
		if i == 0:
			(a, b, c) = F_round(a, b, c, x, 5)
		elif i == 1:
			(a, b, c) = F_round(a, b, c, x, 7)
		else:
		   	(a, b, c) = F_round(a, b, c, x, 9)
		tmpa = a
		a = c
		c = b
		b = tmpa
